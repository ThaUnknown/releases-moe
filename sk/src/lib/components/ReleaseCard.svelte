    ...
    <Card.Footer>
      <div class='grid grid-cols-2 gap-4 w-full'>
        {#each torrents as torrent}
          <Button size='sm' variant='outline' class='px-4' href={torrent.url} target='_blank' class:hidden={PRIVATE_TRACKERS.includes(torrent.tracker)} class:pt-button={PRIVATE_TRACKERS.includes(torrent.tracker)}>
