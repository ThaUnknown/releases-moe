    ...
    <Card.Footer>
      <div class='grid grid-cols-2 gap-4 w-full'>
        {#each torrents as torrent}
          <Button size='sm' variant='outline' class='px-4 {PRIVATE_TRACKERS.includes(torrent.tracker) ? 'pt-button' : ''}' href={!PRIVATE_TRACKERS.includes(torrent.tracker) ? torrent.url : 'javascript:'} data-href={torrent.url} target='_blank'>
